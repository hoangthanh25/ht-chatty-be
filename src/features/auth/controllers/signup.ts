...
export class SignUp {
  ...
  public async create(req: Request, res: Response): Promise<void> {
    ...
    userDataForCache.profilePicture = `https://res.cloudinary.com/dkgspji1r/image/upload/v${result.version}/${userObjectId}`;
    console.log(userDataForCache);
    await userCache.saveUserToCache(`${authObjectId}`, uId, userDataForCache);

    //add to DB
    const userSaveData = omit(userDataForCache, ['uId', 'username', 'email', 'avatarColor', 'password']);
    authQueue.addAuthUserJob('addAuthUserToDB', { value: authData });
    userQueue.addUserJob('addUserToDB', { value: userSaveData });
